...
(function() {
  ...
  async function loadPrefs() {
    try {
      ...
      blurRadius = prefs.privacy_blur_radius || 8;
      maxOverlays = prefs.privacy_blur_max_overlays || 20;
  ...
  function saveOverlays() {
    ...
    const data = overlays.map(o => ({
      left: o.left, top: o.top, width: o.width, height: o.height, type: o.type
    }));
    ...
    try {
      ...
      if (privacy_blur_overlays && Array.isArray(privacy_blur_overlays)) {
        for (const o of privacy_blur_overlays) {
          createBlurOverlay(o.left, o.top, o.width, o.height, o.type, true);
        }
      }
    } catch (err) {}
  }
  ...
  async function createBlurOverlay(left, top, width, height, type = 'auto', restoring = false) {
    ensureShadowHost();

    // Limit overlays
    if (overlays.length >= maxOverlays) {
      announce("Overlay limit reached ("+maxOverlays+"). Remove some overlays.", 3000);
      return;
    }
    ...
    if (!useFallback) {
      ...
      try {
        ...
        overlay.style.background = 'rgba(54,74,94,0.42)';
